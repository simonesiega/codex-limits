...
/** Renders the usage cards currently supplied by the shared core. */
export function UsagePanel({
  ...
  const gutter = stacked || singleCard ? 0 : Math.max(Math.round(bodyWidth * 0.03), 2);
  const cardWidth = singleCard
    ? bodyWidth
    : stacked
      ? Math.max(bodyWidth - 4, 1)
      : Math.max(Math.floor((bodyWidth - gutter) / 2), 1);
  const lastCardWidth =
    stacked || singleCard ? cardWidth : Math.max(bodyWidth - gutter - cardWidth, 1);
  ...
  return (
    <Panel dense={dense} title="Usage Limits" width={width}>
      ...
      >
        {cards.length === 0 ? (
          <Text color={theme.muted}>{emptyLabel}</Text>
        ) : (
          cards.map((card, index) => (
            ...
            >
              <UsageCard
                card={card}
                width={index === cards.length - 1 ? lastCardWidth : cardWidth}
              />
            </Box>
          ))
        )}
