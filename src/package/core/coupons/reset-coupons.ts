/**
 * @fileoverview Shared core logic for reset coupons. This module is part of the canonical data, normalization, or safety layer reused by commands, the TUI, and agent adapters.
 */
...
/** Maps transport details to a safe unavailable coupon result and public warning. */
...
/** Builds the bounded authenticated request shared by coupon reads and redemption. */
...
/** Constructs the minimum credential headers required by the coupon service. */
...
/** Normalizes service-specific redemption outcomes into the stable reset contract. */
...
/** Derives the redemption URL from the configured coupon collection endpoint. */
...
/** Restricts idempotency keys to the bounded format generated by this package. */
function isValidRedeemRequestId(value: string): boolean {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);
}

/** Returns the fail-closed outcome used whenever redemption cannot be verified. */
