...
test("getLiveUsage fetches current usage with Codex credentials", async () => {
  ...
  const fetchMock: FetchLike = async (url, init) => {
    calls.push({ url, authorization: init.headers.Authorization ?? "", accountId: init.headers["ChatGPT-Account-ID"] ?? "" });
  ...
  expect(calls[0]).toEqual({ url: "https://example.test/usage", authorization: "Bearer fake-access-token", accountId: "fake-account-id" });
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        payload: { type: "token_count", rate_limits: { primary: { used_percent: 12, resets_at: 1_767_229_200 }, secondary: { used_percent: 13, resets_at: 1_767_232_800 } } },
      }),
      "utf8",
    );

    const result = await getUsageLimits({
      env: { CODEX_LIMITS_HOME: home, CODEX_LIMITS_ACCESS_TOKEN: "fake-access-token", CODEX_LIMITS_ACCOUNT_ID: "fake-account-id" },
