...
test("readCodexState reads safe JSON and skips sensitive files", async () => {
  ...
  try {
    ...
    await writeFile(join(home, "limits.json"), JSON.stringify({ fiveHour: { remainingPercent: 80 } }), "utf8");
...
test("readCodexSessions sorts all rollout files before parsing", async () => {
  ...
  try {
    ...
    for (let index = 0; index < 60; index += 1) {
      ...
      await writeFile(filePath, JSON.stringify({ type: "event_msg", payload: { type: "token_count", rate_limits: { primary: { used_percent: 80 } } } }), "utf8");
    ...
    await writeFile(
      ...
      JSON.stringify({
        type: "event_msg",
        timestamp: "2026-07-05T10:00:00.000Z",
        payload: { type: "token_count", rate_limits: { primary: { used_percent: 8, resets_at: 1_767_229_200 }, secondary: { used_percent: 9, resets_at: 1_767_232_800 } } },
      }),
      "utf8",
    );
    await utimes(newestPath, new Date("2026-07-05T10:00:00.000Z"), new Date("2026-07-05T10:00:00.000Z"));
