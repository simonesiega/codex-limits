...
test("getResetCoupons rejects fractional and unsafe coupon counts", async () => {
  for (const [availableCount, earnedCount] of [
    [1.5, 2.5],
    ...
    const result = await getResetCoupons({
      ...
      transport: async () => ({
        ...
        payload: {
          ...
          total_earned_count: earnedCount,
          credits: [],
        },
      }),
    });
    ...
    expect(result.warnings).toEqual([
      "Live reset coupon endpoint ignored malformed summary fields.",
    ]);
  }
});
