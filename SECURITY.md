...
The CLI performs bounded, read-only inspection of recognized Codex home candidates. It reads small non-sensitive JSON state files, bounded `sessions/**/rollout-*.jsonl` logs, and `auth.json` only for credential resolution. Traversal depth, directory entries, file counts, file sizes, JSONL line sizes, response sizes, and rendered coupon counts are limited; nested symbolic links are skipped, and each opened state file must retain an identity that resolves inside the detected Codex home. Filesystem diagnostics are path-free, and fallback reset-duration text is accepted only in a compact normalized form. Raw local files, private paths, and credentials are never returned by the public CLI or JSON contracts.
...
- treat closed output pipes as normal pipeline termination and suppress raw stack traces for other stream failures;
